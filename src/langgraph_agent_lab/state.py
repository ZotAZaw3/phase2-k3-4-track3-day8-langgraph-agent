...
class AgentState(TypedDict, total=False):
    ...
    Overwrite fields hold "current truth" (route, attempt, latest decision); append-only
    fields hold history we must not lose (messages, tool results, errors, audit events).
    """
    ...
    # Overwrite: each node replaces the current value, no history needed.
    evaluation_result: str  # "success" | "needs_retry" — gates route_after_evaluate
    pending_question: str | None  # clarification asked back to the user
    proposed_action: str | None  # risky action awaiting approval
    approval: dict[str, Any] | None  # ApprovalDecision payload from the HITL step
